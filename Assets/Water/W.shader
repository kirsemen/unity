		...
		Subshader{
			...
			Pass {
				...
				v2f domain_shader(TessellationFactors tessFactors, const OutputPatch<InternalTessInterp_appdata, 3> vi, float3 bary : SV_DomainLocation) {
					...
					o.ref = ComputeNonStereoScreenPos(UnityObjectToClipPos(vi[0].vertex * bary.x + vi[1].vertex * bary.y + vi[2].vertex * bary.z + (tex2Dlod(_BumpTex, float4(o.uv.xy, 0, 0)).r*2)* float4(0, tex2Dlod(_BumpTex, float4(o.uv.xy, 0, 0)).a, 0, 0)) );
