		...
		Subshader{
			...
			Pass {
				...
				struct v2f {
					...
					float4 reflection_uv : TEXCOORD2;
					#if HAS_REFRACTION
					float4 refraction_uv : TEXCOORD3;
					#endif
				};
				...
				v2f domain_shader(TessellationFactors tessFactors, const OutputPatch<InternalTessInterp_appdata, 3> vi, float3 bary : SV_DomainLocation) {
					...
					o.viewDir = WorldSpaceViewDir((vi[0].vertex * bary.x + vi[1].vertex * bary.y + vi[2].vertex * bary.z).xyzw).xzy;

					o.reflection_uv = ComputeNonStereoScreenPos(UnityObjectToClipPos(
						vi[0].vertex * bary.x + vi[1].vertex * bary.y + vi[2].vertex * bary.z+ _ReflDistort* float4(0, tex2Dlod(_BumpTex, float4(o.uv.xy, 0, 0)).a, 0, 0)
					));
					#if HAS_REFRACTION
					o.refraction_uv = ComputeNonStereoScreenPos(UnityObjectToClipPos(
						vi[0].vertex * bary.x + vi[1].vertex * bary.y + vi[2].vertex * bary.z + _RefrDistort * float4(0, tex2Dlod(_BumpTex, float4(o.uv.xy, 0, 0)).a, 0, 0)
					));
					#endif
					return o;
				}
				...
				half4 frag(v2f i) : SV_Target{
					...
					half3 bump = UnpakeNormalMap(tex2Dlod(_BumpTex, i.uv));
					
					half fresnelFac = dot(i.viewDir, bump);

					half4 color;
					float4 uv1 = i.reflection_uv;
					half4 refl = tex2Dproj(_ReflectionTex, UNITY_PROJ_COORD(uv1));
					#if HAS_REFRACTION
					float4 uv2 = i.refraction_uv;
